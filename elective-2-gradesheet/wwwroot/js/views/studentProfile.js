...
document.addEventListener('DOMContentLoaded', function () {
        ...
        bulkAddForm.addEventListener('submit', async function (event) {
            ...
            try {
                ...
                if (result.success && result.addedCount > 0) {
                    setTimeout(() => {
                        // Get the current URL and extract student ID
                        const currentUrl = window.location.href;
                        const studentIdMatch = currentUrl.match(/StudentProfile\/(\d+)/);
                        if (studentIdMatch) {
                            const studentId = studentIdMatch[1];
                            window.location.href = `/Home/StudentProfile/${studentId}?period=${gradingPeriod}`;
                        } else {
    ...
    var toastMessageFromTempData = document.querySelector('meta[name="toast-message"]')?.content || '';
    var toastTypeFromTempData = document.querySelector('meta[name="toast-type"]')?.content || '';
